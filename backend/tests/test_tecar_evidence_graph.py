...
def test_evidence_graph_assigns_stable_ids_and_links_clusters():
    ...
    clusters = [{
        "cluster_id": "TC0001", "start": 10.0, "end": 12.0, "duration": 2.0,
        "modalities": ["speech", "ocr"], "evidence_count": 2, "evidence": evidence,
    }]
    findings = [{
        "category": "DISCLOSURE", "severity": "HIGH",
        "description": "Please disclose that this is sponsored.", "timestamp": "00:10",
    }]

    graph, normalized = build_evidence_graph(evidence, clusters, findings)

    assert graph["method"] == "TECAR"
    assert graph["node_counts"] == {"evidence": 2, "temporal_cluster": 1, "finding": 1}
    assert [item["evidence_id"] for item in normalized] == ["E0001", "E0002"]
